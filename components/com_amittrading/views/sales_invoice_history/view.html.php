...
class AmittradingViewSales_invoice_history extends JViewLegacy
{
    public function display($tpl = null)
    {
        ...
        */
        ...
        if (!Functions::has_permissions("amittrading", "sales_invoice_history"))
        {
        ...
        $document = JFactory::getDocument()->setTitle("Sales Invoice History");
        ...
        if($customer_id != 0)
        {
            $condition .= ($condition != "" ? " and " : "") . "(si.customer_id=" . $customer_id . ")";
        }
        ...
        if($from_date != "")
        {
            $condition .= ($condition != "" ? " and " : "") . "(si.date='" . date("Y-m-d", strtotime($from_date)) . "')";
        }
       ...
       //$query = "select si.*,c.customer_name,t.transporter_name,vt.vehicle_type,v.vehicle_number from #__sales_invoice si inner join #__customers c on si.customer_id=c.id inner join #__vehicles v on si.vehicle_id=v.id inner join #__transporters t on si.transporter_id=t.id inner join #__vehicles_type vt on si.loading_vehicle_type=vt.id" . ($condition != "" ? " where " . $condition : "") ;
        $query = "select si.*,c.customer_name,vt.vehicle_type,v.vehicle_number from #__sales_invoice si inner join #__customers c on si.customer_id=c.id inner join #__vehicles v on si.vehicle_id=v.id inner join #__vehicles_type vt on si.loading_vehicle_type=vt.id " . ($condition != "" ? " where " . $condition : "") ;
        //echo $query;
