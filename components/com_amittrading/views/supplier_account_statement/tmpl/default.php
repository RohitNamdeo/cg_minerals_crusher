...
<script>
    j(function(){
        j(".date_field").datepicker({"dateFormat" : "dd-M-yy" ,changeMonth: true, changeYear: true});
        j("input[type='button']").button();
    });
    
    function get_records()
    {
        j.get("index.php?option=com_amittrading&view=supplier_account_statement&tmpl=xml&supplier_id=<? echo $this->supplier_id; ?>&from_date=" + j("#from_date").val() + "&to_date=" + j("#to_date").val(), function(data){
            if(data != "")
            {
    ...
    {
        j.get("index.php?option=com_amittrading&view=supplier_account_statement&tmpl=xml&supplier_id=<? echo $this->supplier_id; ?>", function(data){
            if(data != "")
            {
                j("#account_statement").html(j(data).filter("#account_statement").html());
            }
        ...
        if(count($this->account_details) > 0)
        {
            ...
            <table class="clean spread centreheadings" id="account_details">
                ...
                <?
                    ...
                    {
                        //$total_amount = ($details->amount + $details->loading_charges + $details->waiverage_charges);
                        ?>
            ...
            <br /><br />
            <div style="float:right;"><input type="button" value="Print" onclick="window.open('index.php?option=com_amittrading&view=account_statement_print&tmpl=print&type=s&party_id=<? echo $this->supplier_id; ?>&from_date=<? echo $this->from_date; ?>&to_date=<? echo $this->to_date; ?>');go('index.php?option=com_hr&view=dashboard');"></div>
            <br /><br />            
            <?
        }
    ?>
