...
j(function(){
    ...
    {   
        ...
        var gst_percent1 = parseFloat(j(object).parent().parent().find(".gst_percent").val());
        
        var gst_percent2 =  j(".items").find("option:selected").attr("gst_percent");
        ...
        if(gst_percent1 == "" || isNaN(gst_percent1))
        {
            gst_percent1 = 0;   
        }
        ...
        if(gst_percent1 != "")
        {
            var gst_percent = gst_percent1;
        }
        else
        {
            var gst_percent = gst_percent2;        
        }
    ...
    j(document).on("keydown","#party_id,.mixing_note,.item_note",function (e) {
    ...
    function validateForm()
    {
        ...
        j.get("index.php?option=com_amittrading&task=get_bill_no&tmpl=xml&bill_no=" + bill_no + "&supplier_challan_no=" + supplier_challan_no + "&challan_no=" + challan_no + "&purchase_id=" +j("#purchase_id").val(), function(data){
            if(data == "bill_no")
            {  
        ...
        /*var bill_no = j("#bill_no").val();
        j.get("index.php?option=com_amittrading&task=get_bill_no&tmpl=xml&bill_no=" + bill_no + "&purchase_id=" +j("#purchase_id").val(), function(data){
            if(data == "true")
            {  
                alert("Bill number already used.");
                j("#bill_no").focus();
...
<h1>Edit Purchase Entry</h1>

<form method="post" action="index.php?option=com_amittrading&task=update_purchase" id="purchase_invoice" >
<div style="width:100%;">
    <div style="float:left;">
        <table class="clean">
            <tr>
                <td>Purchase Date</td>
                <td><input type="text" name="purchase_date" id="purchase_date" class="custom" value="<? echo date("d-M-Y",strtotime($this->purchase->bill_date)); ?>" readonly="readonly"></td>
            </tr>
            <tr>
                <td>Supplier Name</td>     
                <td>
                    <select id="supplier_id" name="supplier_id" class="custom">
                        ...
                        <?
                          ...
                          {
                              ?>
                              <option value="<? echo $supplier->id; ?>" <? echo ($this->purchase->supplier_id == $supplier->id ? "selected='selected'" : ""); ?> ><? echo $supplier->supplier_name; ?></option>
                              <?
                          }
                        ?>
                    </select>
            ...
            <tr>
                ...
                <td>
                    <select name="bill_type" id="bill_type" tabindex="3" class="custom">
                        ...
                        <option value="<? echo BILL; ?>" <? echo ($this->purchase->bill_type == BILL ? "selected='selected'" : ""); ?> >Bill</option>
                        <option value="<? echo CHALLAN; ?>" <? echo ($this->purchase->bill_type == CHALLAN ? "selected='selected'" : ""); ?>>Challan</option>
                    </select>
                </td>
            </tr>
            <tr>
                <td>Bill No</td>
                <td><input type="text" name="bill_no" id="bill_no" class="custom" value="<? echo $this->purchase->bill_no ;?>"></td>
            </tr>
            <tr>
                <td>Supplier Challan No</td>
                <td><input type="text" name="supplier_challan_no" id="supplier_challan_no" class="custom" value="<? echo $this->purchase->supplier_challan_no; ?>"></td>
            </tr>
            <tr>
                <td>Challan No</td>
                <td><input type="text" name="challan_no" id="challan_no" class="custom" value="<? echo $this->purchase->challan_no; ?>"></td>
            </tr>
        </table>
        </div>
    <div style="float:left;margin-right:20px;margin-left:20px;">
        <table class="clean centreheadings">
            <tr>
                <td>Vehicle No</td>
                <td>
                    <select id="vehicle_id" name="vehicle_id" class="custom">
                    <option></option>
                     <?
                      foreach($this->vehicles as $vehicle)
                      { 
                          ?>
                          <option value="<? echo $vehicle->id; ?>" <? echo ($this->purchase->vehicle_id == $vehicle->id ? "selected='selected'" : ""); ?> ><? echo $vehicle->vehicle_number; ?></option>
                          <?
                      }
                    ?>
                </select>
            ...
            <tr>
                ...
                <td><input type="text" name="loading_charges" id="loading_charges" class="custom" value="<? echo $this->purchase->loading_charges; ?>"></td>
             </tr>
             <tr>
                <td>Waiverage Charges</td>     
                <td><input type="text" name="waiverage_charges" id="waiverage_charges" class="custom" value="<? echo $this->purchase->waiverage_charges; ?>"></td>
             </tr>              
             <tr>
                <td>Remarks</td>     
                <td><input type="text" name="remarks" id="remarks" class="custom" value="<? echo $this->purchase->remarks; ?>"></td>
             </tr>
        </table>
    </div>
        ...
        <table class="clean centreheadings">
                        ...
                        foreach($this->purchase_items as $item)
                        {
                            ?>
                             <tr>
                                <td id="idd"></td>
                                <td>
                                    <select name="item_id[]" class="items" id="items" style="width:200px;">
                                        <option></option>
                                        <?
                                        if(count($this->products) > 0)
                                        {
                                            ...
                                            {
                                                ?><option value="<? echo $product->id; ?>" gst_percent="<? echo $product->gst_percent;?>" unit_id="<? echo $product->unit_id;?>"  unit_name="<? echo $product->unit_name;?>" <? echo ($item->item_id == $product->id ? "selected='selected'" : "");?>><? echo $product->product_name; ?></option><?
                                            }
                                        }
                                    ?>
                                    </select>
                                ...
                                <td>
                                    <span class="unit_id"><? echo $item->unit;?></span>
                                    <input type="hidden" name="unit_id[]" class="unit_id" id="unit_id" value="<? echo $product->unit_id;?>">    
                                </td>
                                <td><input type="text" name="product_mt[]" size="6%" class="product_mt" value="<? echo $item->product_mt;?>"></td>
                                ...
                                <td class="gst_fields" align="right">
                                    ...
                                    <input type="hidden" name="gst_percent[]" class="gst_percent" value="<? echo $item->gst_percent;?>">
                                </td>
                                <td class="gst_fields">
                                    <input type="text" name="gst_amount[]" class="gst_amount" readonly="readonly" style="text-align:right;" value="<? echo $item->gst_amount;?>">
                                </td>
                                <td><input type="text" name="product_total_amount[]" class="product_total_amount" readonly="readonly" style="text-align:right;" value="<? echo $item->total_amount;?>"></td>
                                <td><input type="text" name="item_note[]"  class="item_note add_items" value="<? echo $item->note;?>"></td>
            ...
            <tfoot>
                <tr>
                    ...
                    <td id="product_total_amt" align="right"><? echo floatval($this->purchase->total_amount -($this->purchase->loading_charges + $this->purchase->waiverage_charges)); ?></td>
                    <td colspan="2"></td>
                </tr>
                <tr>
    ...
    <input type="hidden" name="purchase_id" id="purchase_id" value=" <? echo intval($this->purchase->id); ?>">
    ...
    <tbody id="dummy_item">
         <tr>
            ...
            <td class="gst_fields" align="right">
                ...
                <input type="hidden" name="gst_percent[]" class="gst_percent" >
            </td>
            <td class="gst_fields">
                <input type="text" name="gst_amount[]" class="gst_amount" readonly="readonly" style="text-align:right;">
            </td>
            <td><input type="text" name="product_total_amount[]" class="product_total_amount" readonly="readonly" style="text-align:right;"></td>
            <td><input type="text" name="item_note[]"  class="item_note add_items"></td>
            <td align="center"><img src="custom/graphics/icons/blank.gif" class="delete product_delete" title="Delete Row" ></td>   
        </tr>
    </tbody>
</table>
