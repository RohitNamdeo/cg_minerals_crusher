...
var defaultConfig = {
    ...
    //columnWidths:[30,120,80,90,100,80,50]//array for customizing the columns widths. Example: columnWidths:[20,100,100]. all columns in table should be defined
};
...
jsPDFAPI.tableToJson = function(id) {
	...
	var noOfCells = table.rows[0].cells.length;// no of colums
...
function calculateDim(data, dimensions) {
	...
	for (var i = 0; i < lengths.length; i++) {
		if ((lengths[i] * (fontSize)) > (width - dimensions[5])) {
			...
			heights[i] = (noOfLines) * (fontSize / 2) + dimensions[6] + 5;
		} else {
			heights[i] = (fontSize + (fontSize / 2)) + dimensions[6] + 5;
		}
	}
...
// header and border column color
