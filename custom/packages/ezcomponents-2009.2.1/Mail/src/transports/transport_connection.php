    ...
    public function getLine( $trim = false )
    {
        ...
        if ( is_resource( $this->connection ) )
        {
            ...
            while ( strpos( $data, self::CRLF ) === false )
            {
                ...
                //HIGHLY risky line, may start causing issue with future mails too
                //The lines were added to force RFC822 compliance as I came across
                //some email with LF only line endings instead of CRLF line endings
                //as defined in RFC822
                //Added by Kunal Gala on 23-Apr 2012
                $line = str_replace("\r\n", "\n", $line);
                $line = str_replace("\n", "\r\n", $line);
                //End of HIGH risk lines
                $data .= $line;
            }
